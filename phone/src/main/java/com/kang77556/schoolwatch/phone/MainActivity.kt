...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        sendButton = Button(this).apply {
            text = "워치로 보내기"
            setOnClickListener { currentJson?.let(::sendToWatch) }
        }
        applyButton = Button(this).apply {
            text = "워치에 적용"
            setOnClickListener { currentJson?.let(::sendThenOpenWatchFaceSelection) }
        }
    ...
    private fun sendThenOpenWatchFaceSelection(json: String) {
        ...
        WearSync.send(this, json) { ok ->
            runOnUiThread {
                if (!ok) {
                    status.text = "워치 전송에 실패했습니다. 연결을 확인해 주세요."
