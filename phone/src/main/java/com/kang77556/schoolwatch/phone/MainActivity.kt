...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val importButton = Button(this).apply {
            ...
            setOnClickListener {
                try {
                    jsonPicker.launch(JsonPickerContract.mimeTypes())
                } catch (e: Exception) {
                    status.text = "파일 선택기를 열지 못했습니다: ${e.javaClass.simpleName}"
                }
            }
        }
    ...
    private fun sendThenOpenWatchFaceSelection(json: String) {
        ...
        WearSync.send(this, json) { ok -> runOnUiThread {
            ...
            status.text = "전송 완료. 워치페이스 선택 화면을 엽니다."
            openWearableSelection()
        } }
    }

    private fun openWearableSelection() {
        val candidates = listOf(
            Intent(Intent.ACTION_VIEW, Uri.parse("wearable://watchfaces")),
            packageManager.getLaunchIntentForPackage("com.samsung.android.app.watchmanager"),
            Intent(Settings.ACTION_BLUETOOTH_SETTINGS)
        ).filterNotNull()
        val target = candidates.firstOrNull { it.resolveActivity(packageManager) != null }
        if (target != null) {
            try { startActivity(target) } catch (e: Exception) { status.text = "워치페이스 화면을 열지 못했습니다: ${e.javaClass.simpleName}" }
    ...
    private fun updateButtons() {
        val enabled = WatchApplyState.canApply(currentJson)
