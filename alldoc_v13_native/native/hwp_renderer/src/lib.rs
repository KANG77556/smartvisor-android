...
use jni::sys::{jboolean, jbyteArray, jdoubleArray, jint, jlong, jstring};
use jni::JNIEnv;
use std::fs;
...
fn service_error(error: NativeServiceError) -> String {
    match error {
        ...
        NativeServiceError::Edit(message) => format!("EDIT_FAILED|{message}"),
    }
}

fn throw(env: &mut JNIEnv, encoded: impl AsRef<str>) {
    let _ = env.throw_new("com/milsung/alldocviewer/hwp/HwpNativeException", encoded.as_ref());
}
...
#[no_mangle]
pub extern "system" fn Java_com_milsung_alldocviewer_hwp_NativeHwpBridge_nativeVersion(env: JNIEnv, _class: JClass) -> jstring {
...
#[no_mangle]
pub extern "system" fn Java_com_milsung_alldocviewer_hwp_NativeHwpBridge_nativeOpenDocument(mut env: JNIEnv, _class: JClass, path: JString) -> jlong {
    let result = catch_unwind(AssertUnwindSafe(|| -> Result<jlong, String> {
        let path: String = env.get_string(&path).map_err(|e| format!("INVALID_ARGUMENT|invalid path: {e}"))?.into();
        if path.is_empty() { return Err("INVALID_ARGUMENT|empty path".to_string()); }
        let metadata = fs::metadata(&path).map_err(|e| format!("OPEN_FAILED|metadata: {e}"))?;
        if metadata.len() > MAX_HWP_BYTES { return Err(format!("OPEN_FAILED|file exceeds {} bytes", MAX_HWP_BYTES)); }
...
#[no_mangle]
pub extern "system" fn Java_com_milsung_alldocviewer_hwp_NativeHwpBridge_nativeCloseDocument(mut env: JNIEnv, _class: JClass, handle: jlong) {
...
#[no_mangle]
pub extern "system" fn Java_com_milsung_alldocviewer_hwp_NativeHwpBridge_nativeGetPageCount(mut env: JNIEnv, _class: JClass, handle: jlong) -> jint {
...
#[no_mangle]
pub extern "system" fn Java_com_milsung_alldocviewer_hwp_NativeHwpBridge_nativeGetPageInfo(mut env: JNIEnv, _class: JClass, handle: jlong, page_index: jint) -> jdoubleArray {
    ...
    match result {
        Ok(Ok((width, height))) => match env.new_double_array(2) {
            Ok(array) => {
                if env.set_double_array_region(&array, 0, &[width, height]).is_ok() { array.into_raw() }
                else { throw(&mut env, "JNI_ERROR|failed to fill page info"); ptr::null_mut() }
            }
...
#[no_mangle]
pub extern "system" fn Java_com_milsung_alldocviewer_hwp_NativeHwpBridge_nativeRenderPagePng(mut env: JNIEnv, _class: JClass, handle: jlong, page_index: jint, scale: f64, max_dimension: jint, max_pixels: jlong) -> jbyteArray {
    let result = catch_unwind(AssertUnwindSafe(|| -> Result<Vec<u8>, String> {
        if handle <= 0 { return Err("INVALID_HANDLE|handle must be positive".to_string()); }
        if page_index < 0 || max_pixels <= 0 { return Err("INVALID_ARGUMENT|invalid page index or pixel limit".to_string()); }
...
#[no_mangle]
pub extern "system" fn Java_com_milsung_alldocviewer_hwp_NativeHwpBridge_nativeRenderPageSvg(mut env: JNIEnv, _class: JClass, handle: jlong, page_index: jint) -> jstring {
    let result = catch_unwind(AssertUnwindSafe(|| -> Result<String, String> {
        if handle <= 0 { return Err("INVALID_HANDLE|handle must be positive".to_string()); }
        if page_index < 0 { return Err("INVALID_ARGUMENT|page index must be non-negative".to_string()); }
        with_service(|service| service.render_page_svg(handle as u64, page_index as u32).map_err(service_error))
    }));
    match result {
        Ok(Ok(svg)) => match env.new_string(svg) {
            Ok(s) => s.into_raw(),
            Err(e) => { throw(&mut env, format!("JNI_ERROR|svg string: {e}")); ptr::null_mut() }
        },
        Ok(Err(message)) => { throw(&mut env, message); ptr::null_mut() }
        Err(_) => { throw(&mut env, "NATIVE_PANIC|native SVG render panic"); ptr::null_mut() }
    }
}

#[no_mangle]
pub extern "system" fn Java_com_milsung_alldocviewer_hwp_NativeHwpBridge_nativeExportHwpx(mut env: JNIEnv, _class: JClass, handle: jlong) -> jbyteArray {
    let result = catch_unwind(AssertUnwindSafe(|| -> Result<Vec<u8>, String> {
        if handle <= 0 { return Err("INVALID_HANDLE|handle must be positive".to_string()); }
...
#[no_mangle]
pub extern "system" fn Java_com_milsung_alldocviewer_hwp_NativeHwpBridge_nativeReplaceAll(mut env: JNIEnv, _class: JClass, handle: jlong, find: JString, replace: JString, case_sensitive: jboolean) -> jstring {
    let result = catch_unwind(AssertUnwindSafe(|| -> Result<String, String> {
        if handle <= 0 { return Err("INVALID_HANDLE|handle must be positive".to_string()); }
        let find: String = env.get_string(&find).map_err(|e| format!("INVALID_ARGUMENT|invalid find text: {e}"))?.into();
        let replace: String = env.get_string(&replace).map_err(|e| format!("INVALID_ARGUMENT|invalid replacement text: {e}"))?.into();
        if find.is_empty() { return Err("INVALID_ARGUMENT|find text must not be empty".to_string()); }
        with_service(|service| service.replace_all(handle as u64, &find, &replace, case_sensitive != 0).map_err(service_error))
    }));
    match result {
        Ok(Ok(json)) => match env.new_string(json) {
            Ok(s) => s.into_raw(),
            Err(e) => { throw(&mut env, format!("JNI_ERROR|replace result string: {e}")); ptr::null_mut() }
        },
        Ok(Err(message)) => { throw(&mut env, message); ptr::null_mut() }
        Err(_) => { throw(&mut env, "NATIVE_PANIC|native replace panic"); ptr::null_mut() }
    }
}
